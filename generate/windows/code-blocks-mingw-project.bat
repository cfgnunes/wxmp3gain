...
set "buildFolder=../build/code-blocks_win"
...
REM ======================================================================
REM Create a project for Code::Blocks
REM ======================================================================

REM Inform the user
echo ===========================================================================================
echo Building project(s) for Code::Blocks
echo ===========================================================================================

REM Go into the source folder.
cd ../../sources

REM Create projects for different address models, target processors and build types.
for %%a in ("%addressModel:,=" "%") do (
	...
	for %%p in ("%targetProcessorArchitecture:,=" "%") do (
		...
		for %%b in ("%buildTypes:,=" "%") do (
			...
			cmake -B!buildFolder! -G"CodeBlocks - MinGW Makefiles" !cmakeToolchainFile! %cmakeLogLevel% !cmakeBuildType!
			
			REM Inform the user
			echo -------------------------------------------------------------------------------------------
			echo Done building. Import the Code::Blocks project from the subfolder "!buildFolder!"!
			echo -------------------------------------------------------------------------------------------
		)
	)
)
